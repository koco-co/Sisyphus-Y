...
- [x] **ANA-05**: 覆盖追踪 Tab：需求点 × 场景类型矩阵视图，展示当前用例覆盖情况
- [x] **ANA-06**: 分析结果刷新后完整恢复（持久化到 DB）
- [x] **ANA-07**: 存在未处理高风险遗漏项时「进入工作台」按钮置灰，hover 提示文案

### 工作台（M05 重构）

- [x] **WRK-01**: 步骤条固定顶部，显示「Step 1 确认测试点 → Step 2 生成用例」，当前步骤高亮
...
- [x] **TC-12**: 导出支持 xlsx/csv/xmind/md 四种格式
- [x] **TC-13**: 导出范围：当前目录/按需求/按迭代/自由勾选（四种）
- [x] **TC-14**: 导出自定义字段（复选框选择，XMind 格式固定全字段）
...
- [x] **DSH-02**: 中部折线图：近6迭代用例总量/P0覆盖率/缺陷发现率，三线可单独显示
...
| TC-12 | Phase 4 | Complete |
| TC-13 | Phase 4 | Complete |
| TC-14 | Phase 4 | Complete |
