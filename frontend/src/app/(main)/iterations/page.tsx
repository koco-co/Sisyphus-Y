...
import { Table, Modal, Input, DatePicker, Form, message, Popconfirm, Skeleton, Empty } from "antd";
...
function IterationsContent() {
  ...
  const { data: product } = useQuery<Product>({
    ...
    queryFn: () => apiClient.get(`/products`).then((list: unknown) => (list as Product[]).find((p) => p.id === productId)!),
    enabled: !!productId,
  });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, ...data }: { id: string; name?: string; start_date?: string; end_date?: string; status?: string }) =>
      ...
      <div className="no-sidebar">
        <div style={{ maxWidth: 1200, margin: "0 auto", paddingTop: 80 }}>
          <Empty description="请先选择一个子产品">
            <button className="btn btn-primary" onClick={() => router.push("/products")}>前往子产品管理</button>
          </Empty>
        </div>
      </div>
    );
  ...
  const handleCreateFinish = (values: Record<string, unknown>) => {
    const payload: { name: string; start_date?: string; end_date?: string } = { name: values.name as string };
    if (values.start_date) payload.start_date = (values.start_date as { format: (f: string) => string }).format("YYYY-MM-DD");
    if (values.end_date) payload.end_date = (values.end_date as { format: (f: string) => string }).format("YYYY-MM-DD");
    createMutation.mutate(payload);
  };

  const handleEditFinish = (values: Record<string, unknown>) => {
    if (!editItem) return;
    const payload: { id: string; name?: string; start_date?: string; end_date?: string } = { id: editItem.id, name: values.name as string };
    if (values.start_date) payload.start_date = (values.start_date as { format: (f: string) => string }).format("YYYY-MM-DD");
    if (values.end_date) payload.end_date = (values.end_date as { format: (f: string) => string }).format("YYYY-MM-DD");
  ...
  const columns: ColumnsType<Iteration> = [
    {
      ...
      render: (name: string, record) => (
        <a
          onClick={(e) => { e.stopPropagation(); router.push(`/requirements?iterationId=${record.id}`); }}
    ...
    },
    {
      ...
      key: "start_date",
      width: 140,
      render: (val: string | null) => val ? <span className="mono">{val}</span> : <span style={{ color: "var(--text3)" }}>—</span>,
    },
    {
      title: "结束日期",
      dataIndex: "end_date",
      ...
      render: (_, record) => (
        <div style={{ display: "flex", gap: 8 }}>
          <button
            ...
            onClick={(e) => { e.stopPropagation(); setEditItem(record); editForm.setFieldsValue({ name: record.name }); }}
            title="编辑"
          >
            <Pencil size={14} />
          </button>
    ...
    <div className="no-sidebar">
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div className="topbar">
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
              <button className="btn btn-ghost btn-sm" onClick={() => router.push("/products")} title="返回子产品列表">
                <ArrowLeft size={16} />
              </button>
        ...
        <div className="card" style={{ padding: 0, overflow: "hidden" }}>
          {isLoading ? (
            <div style={{ padding: 24 }}><Skeleton active paragraph={{ rows: 5 }} title={false} /></div>
          ) : (
            <Table<Iteration>
              columns={columns}
      ...
      <Modal
        ...
        cancelText="取消"
      >
        <Form form={createForm} layout="vertical" onFinish={handleCreateFinish} style={{ marginTop: 16 }}>
      ...
      <Modal
        ...
        cancelText="取消"
      >
        <Form form={editForm} layout="vertical" onFinish={handleEditFinish} style={{ marginTop: 16 }}>
          <Form.Item name="name" label="迭代名称" rules={[{ required: true, message: "请输入迭代名称" }]}>
            <Input />
          </Form.Item>
          <Form.Item name="start_date" label="开始日期">
...
export default function IterationsPage() {
  return (
    <Suspense fallback={<div className="no-sidebar"><div style={{ maxWidth: 1200, margin: "0 auto" }}><div className="card"><Skeleton active paragraph={{ rows: 6 }} title={false} /></div></div></div>}>
      <IterationsContent />
    </Suspense>
  );
}
