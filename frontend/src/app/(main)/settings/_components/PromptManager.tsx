...
export function PromptManager() {
  ...
  const loadHistory = useCallback(async (moduleKey: string) => {
    try {
      const data = await api.get<PromptHistoryItem[]>(`/ai-config/prompts/${moduleKey}/history`);
      setHistory(data);
    } catch {
      setHistory([]);
    }
    ...
    <div>
      <div className="alert-banner mb-4">
        ...
        <span className="text-[12.5px]">
          修改 System Prompt 会直接影响 AI
          输出质量，建议在测试环境验证后再用于生产。每个模块保留最近 5 次修改记录，可随时回滚。
        </span>
      </div>
      ...
      <div className="card p-4">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h3 className="text-[14px] font-semibold text-text">{currentModule.label}</h3>
            <p className="text-[12px] text-text3 mt-0.5">{currentModule.description}</p>
            {defaultModules.has(activeModule) && (
              <span className="text-[10px] text-text3 bg-bg2 px-1.5 py-0.5 rounded font-mono">
                默认值
              </span>
            )}
          </div>
      ...
      {showHistory && (
        <div className="card p-4 mt-4">
          <h4 className="text-[13px] font-medium text-text mb-3">历史版本（最近 5 条）</h4>
          {history.length === 0 ? (
            <p className="text-[12px] text-text3">暂无历史版本</p>
          ) : (
            <div className="flex flex-col gap-2">
              {history.map((h) => (
                <div key={h.id} className="flex items-center justify-between p-3 bg-bg2 rounded-md">
                  <div>
                    <span className="text-[12px] font-mono text-text2">v{h.version}</span>
