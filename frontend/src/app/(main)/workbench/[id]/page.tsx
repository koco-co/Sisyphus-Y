...
export default function WorkbenchPage() {
  ...
  const acceptMutation = useMutation({
    mutationFn: (caseId: string) => apiClient(`/generation/sessions/${sessionId}/cases/${caseId}/accept`, { method: 'POST' }),
    ...
    <div className="flex flex-col h-[calc(100vh-0px)] overflow-hidden">
      ...
      <div className="flex-1 grid grid-cols-[240px_1fr_340px] min-h-0">
        ...
        <div className="bg-bg1 border-r border-border p-3 overflow-y-auto">
          <div className="text-[11px] font-semibold text-text3 uppercase tracking-wide mb-2">需求上下文</div>
        ...
        <div className="flex flex-col bg-bg overflow-hidden">
          ...
          <div className="border-t border-border p-3 flex gap-2">
            ...
            />
            <button type="button" onClick={sendMessage} disabled={isStreaming || !input.trim()} className="px-5 py-2 rounded-md text-[12px] font-semibold bg-accent text-black disabled:opacity-50 hover:bg-accent2 transition-colors">
              生成
            </button>
          </div>
        </div>
