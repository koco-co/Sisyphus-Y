...
function WorkbenchPageContent() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // hydration error handled silently
      }
    })();
   ...
   * 直接从 DB 加载真实用例，避免 fake ID 闪烁和采纳按钮失效
   */
  const handleGenerationComplete = useCallback(
    ...
    [wb.selectedReqId, wb.loadTestCases],
  );
...
function WorkbenchPageFallback() {
  return (
    <div className="flex h-screen items-center justify-center bg-sy-bg text-[13px] text-sy-text-3">
      正在加载工作台...
    </div>
  );
}
